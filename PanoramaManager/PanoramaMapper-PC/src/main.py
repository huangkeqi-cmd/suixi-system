                        ...
                        try:
                            ...
                            if marker_id and direction_str is not None:
                                ...
                                if parent_app and parent_app.project_data and parent_app.project_dir:
                                    ...
                                    # 5. 触发查看器 HTML 同步更新（区别于实时同步）
                                    parent_app.direction_saved.emit(marker_id, direction)
...
class FloatingToolbar(QFrame):
    ...
    BUTTON_CONFIG = {
        'photo_relation': {'text': '调整关联', 'tooltip': '调整已关联照片对应关系（单点互换/批量重排/跨楼层）'},
    ...
    photo_relation_clicked = pyqtSignal()
    ...
    def update_settings(self, settings):
        ...
        if parent:
            if self._settings.get('orientation', 'vertical') == 'horizontal':
                ...
                x = max(10, parent.width() - 110 - 10)   # ← 改：90→110
                self.move(x, self.y())
    ...
    def _rebuild_layout(self):
        ...
        if is_vertical:
            ...
            if parent:
                x = max(10, parent.width() - 110 - 10)   # ← 110（改自90）
                y = self.y() if self.y() > 0 else 70
                self.setGeometry(x, y, 110, 300)         # ← 宽110，高300（改自90×230）
            else:
                self.setGeometry(self.x(), self.y(), 110, 300)
        else:
            layout = QHBoxLayout(self)
            if parent:
                x = 10
                y = 130
                self.setGeometry(x, y, 540, 55)          # ← 宽540，高55（改自380×50）
            else:
                self.setGeometry(self.x(), self.y(), 540, 55)
    ...
    def _on_button_clicked(self, key: str):
        ...
        elif key == 'photo_relation': 
            self.photo_relation_clicked.emit()
...
class PhotoReorderDialog(QDialog):
    """自定义照片顺序对话框 - 拖拽排序照片后分配给固定采集点"""

    def __init__(self, markers_data: List[dict], parent=None):
        """
        markers_data: 每个元素为 dict，包含:
            - marker_id, marker_name, floor_name
            - photo_filename, panoramaPath, originalPhotoPath, photos, cameraFileName
        """
        super().__init__(parent)
        self.setWindowTitle("自定义照片重排")
        self.resize(700, 500)
        self._markers_data = markers_data  # 原始采集点数据（固定顺序）
        self._photos_data = []  # 照片数据（可排序）
        self._init_ui()
        self._extract_photos()
    ...
    def _init_ui(self):
        ...
        hint = QLabel("💡 左侧为固定采集点顺序，右侧照片可拖拽调整顺序，确认后按新顺序分配")
        hint.setStyleSheet("color: #8E8E93; font-size: 12px;")
        layout.addWidget(hint)
        ...
        # 左侧：采集点列表（固定）
        left_group = QGroupBox("采集点（固定顺序）")
        left_layout = QVBoxLayout(left_group)
        self.marker_list = QListWidget()
        self.marker_list.setDragEnabled(False)
        self.marker_list.setAcceptDrops(False)
        ...
        # 右侧：照片列表（可拖拽排序）
        right_group = QGroupBox("照片（拖拽调整顺序）")
        ...
        reverse_btn.setStyleSheet("""
            ...
            QPushButton:hover { background-color: #3f3ea8; }
        """)
        reverse_btn.clicked.connect(self._reverse_photos)
        btn_layout.addWidget(reverse_btn)

        reset_btn = QPushButton("↩️ 恢复原始")
        reset_btn.setStyleSheet("""
            QPushButton {
                padding: 6px 12px; font-size: 12px;
                background-color: #8E8E93; color: white;
                border: none; border-radius: 6px;
            }
            QPushButton:hover { background-color: #636366; }
        """)
        reset_btn.clicked.connect(self._reset_photos)
    ...
    def _extract_photos(self):
        ...
        for idx, m in enumerate(self._markers_data, 1):
            # 左侧采集点
            item_text = f"{idx}. [{m['floor_name']}] {m['marker_name']}\n   当前: {m['photo_filename'] or '无'}"
            item = QListWidgetItem(item_text)
            item.setData(Qt.ItemDataRole.UserRole, m['marker_id'])
            self.marker_list.addItem(item)

            # 右侧照片
            photo_info = {
                'marker_id': m['marker_id'],
                'panoramaPath': m['panoramaPath'],
                'originalPhotoPath': m['originalPhotoPath'],
                'photos': m['photos'],
                'cameraFileName': m['cameraFileName'],
            }
            self._photos_data.append(photo_info)

            p_item = QListWidgetItem(f"{idx}. {m['photo_filename'] or '无照片'}")
            p_item.setData(Qt.ItemDataRole.UserRole, idx - 1)  # 原始索引
            self.photo_list.addItem(p_item)

    def _reverse_photos(self):
        """反转照片顺序"""
        items = []
        for i in range(self.photo_list.count()):
            items.append(self.photo_list.takeItem(0))
        for item in reversed(items):
            self.photo_list.addItem(item)

    def _reset_photos(self):
        """恢复原始照片顺序"""
        self._extract_photos()

    def get_new_order(self) -> List[dict]:
        """返回新的照片顺序列表，与采集点列表一一对应"""
        new_order = []
        for i in range(self.photo_list.count()):
            orig_idx = self.photo_list.item(i).data(Qt.ItemDataRole.UserRole)
            new_order.append(self._photos_data[orig_idx])
        return new_order


class PhotoRelationManagerDialog(QDialog):
    """照片关联关系管理器 - 单点互换、批量重排、跨楼层互换"""

    def __init__(self, project_data: Project, current_floor_id: str, parent=None):
        super().__init__(parent)
        self.setWindowTitle("📷 照片关联关系调整")
        self.resize(900, 650)
        self.project_data = project_data
        self.current_floor_id = current_floor_id
        self.parent_manager = parent
        self._all_linked_markers: List[dict] = []  # 扁平化的所有已关联采集点
        self._init_ui()
        self._refresh_table()
    ...
    def _init_ui(self):
        ...
        # 标题与说明
        title = QLabel("调整已关联采集点与照片的对应关系")
        title.setStyleSheet("font-size: 16px; font-weight: bold;")
        layout.addWidget(title)

        hint = QLabel("💡 勾选需要调整的采集点，使用下方按钮进行互换、循环移位或自定义重排。支持跨楼层操作。")
        hint.setStyleSheet("color: #8E8E93; font-size: 12px;")
        ...
        filter_layout.addWidget(QLabel("显示范围:"))
        self.filter_combo = QComboBox()
        self.filter_combo.addItem("全部楼层", "all")
        if self.project_data and self.project_data.floors:
            for floor in sorted(self.project_data.floors, key=lambda f: f.get('order', 0), reverse=True):
                self.filter_combo.addItem(floor.get('name', '未命名'), floor.get('id'))
        self.filter_combo.addItem("仅当前楼层", "current")
        self.filter_combo.currentIndexChanged.connect(self._refresh_table)
        filter_layout.addWidget(self.filter_combo)
        filter_layout.addStretch()

        self.show_unlinked_cb = QCheckBox("显示未关联采集点（可参与接收照片）")
        self.show_unlinked_cb.setChecked(False)
        self.show_unlinked_cb.stateChanged.connect(self._refresh_table)
        filter_layout.addWidget(self.show_unlinked_cb)
        ...
        self.table.setHorizontalHeaderLabels(["选择", "楼层", "点位名称", "点位ID", "当前照片", "状态"])
        ...
        # 操作按钮区
        ops_group = QGroupBox("批量操作")
        ops_layout = QHBoxLayout(ops_group)

        # 单点/多点互换
        swap_btn = QPushButton("互换照片")
        swap_btn.setToolTip("选中两行，互换它们的照片关联")
        ...
        ops_layout.addWidget(swap_btn)

        # 循环左移
        shift_left_btn = QPushButton("⬅️ 循环左移")
        shift_left_btn.setToolTip("选中多行，照片循环向左移动一位")
        shift_left_btn.setStyleSheet("""
            QPushButton {
                padding: 8px 16px; font-size: 13px;
        ...
        """)
        shift_left_btn.clicked.connect(lambda: self._on_cycle_shift(-1))
        ops_layout.addWidget(shift_left_btn)

        # 循环右移
        shift_right_btn = QPushButton("➡️ 循环右移")
        shift_right_btn.setToolTip("选中多行，照片循环向右移动一位")
        shift_right_btn.setStyleSheet("""
            QPushButton {
                padding: 8px 16px; font-size: 13px;
                background-color: #FF9500; color: white;
                border: none; border-radius: 6px;
            ...
            QPushButton:hover { background-color: #B36800; }
        """)
        shift_right_btn.clicked.connect(lambda: self._on_cycle_shift(1))
        ops_layout.addWidget(shift_right_btn)

        # 自定义重排
        custom_btn = QPushButton("📋 自定义重排")
        custom_btn.setToolTip("打开拖拽排序对话框，自定义照片分配顺序")
        custom_btn.setStyleSheet("""
            ...
            QPushButton:hover { background-color: #248A3D; }
        """)
        custom_btn.clicked.connect(self._on_custom_reorder)
        ops_layout.addWidget(custom_btn)

        # 全选/反选
        select_all_btn = QPushButton("☑️ 全选")
        select_all_btn.clicked.connect(self._select_all)
        ops_layout.addWidget(select_all_btn)
        ...
        preview_btn = QPushButton("👁️ 预览变更")
        preview_btn.setStyleSheet("""
            QPushButton {
                padding: 8px 20px; font-size: 13px;
                background-color: #5AC8FA; color: white;
                border: none; border-radius: 6px;
            }
        ...
        ok_btn.setStyleSheet("""
            ...
            QPushButton:hover { background-color: #0056CC; }
        """)
        ok_btn.clicked.connect(self._save_changes)
        bottom_btn = QPushButton("取消")
        bottom_btn.clicked.connect(self.reject)
        bottom_layout.addWidget(ok_btn)
        bottom_layout.addWidget(bottom_btn)
        layout.addLayout(bottom_layout)
    ...
    def _collect_markers(self) -> List[dict]:
        ...
        show_unlinked = self.show_unlinked_cb.isChecked()

        for floor in self.project_data.floors:
            floor_id = floor.get('id', '')
            floor_name = floor.get('name', '未命名')

            # 筛选
            if filter_val == 'current' and floor_id != self.current_floor_id:
                continue
            if filter_val != 'all' and filter_val != 'current' and floor_id != filter_val:
                continue

            for marker in floor.get('markers', []):
                status = marker.get('status', '')
                if status == 'linked' or (show_unlinked and status in ('pending', 'captured', 'missing')):
                    result.append({
                        'marker': marker,
                        'floor_id': floor_id,
                        'floor_name': floor_name,
    ...
    def _refresh_table(self):
        """刷新表格显示"""
        self._all_linked_markers = self._collect_markers()
        self.table.setRowCount(len(self._all_linked_markers))

        for row, item in enumerate(self._all_linked_markers):
            marker = item['marker']
            ...
            # 楼层
            floor_item = QTableWidgetItem(item['floor_name'])
            floor_item.setData(Qt.ItemDataRole.UserRole, row)  # 存储行索引
            floor_item.setFlags(floor_item.flags() & ~Qt.ItemFlag.ItemIsEditable)
            self.table.setItem(row, 1, floor_item)

            # 点位名称
            name_item = QTableWidgetItem(marker.get('customName', '') or marker.get('id', ''))
            name_item.setFlags(name_item.flags() & ~Qt.ItemFlag.ItemIsEditable)
            self.table.setItem(row, 2, name_item)

            # 点位ID
            id_item = QTableWidgetItem(marker.get('id', ''))
            id_item.setFlags(id_item.flags() & ~Qt.ItemFlag.ItemIsEditable)
            self.table.setItem(row, 3, id_item)

            # 当前照片
            photo_name = marker.get('cameraFileName', '') or os.path.basename(marker.get('panoramaPath', '') or '无')
            photo_item = QTableWidgetItem(photo_name)
            photo_item.setFlags(photo_item.flags() & ~Qt.ItemFlag.ItemIsEditable)
            self.table.setItem(row, 4, photo_item)

            # 状态
            status_map = {'linked': '✅ 已关联', 'pending': '⏳ 待采集', 'captured': '📷 已采集', 'missing': '❌ 缺失'}
            status_item = QTableWidgetItem(status_map.get(marker.get('status', ''), marker.get('status', '')))
    ...
    def _swap_photo_data(self, m1: dict, m2: dict):
        """互换两个采集点的照片关联字段"""
        fields = ['panoramaPath', 'originalPhotoPath', 'photos', 'cameraFileName']
        for f in fields:
            m1[f], m2[f] = m2[f], m1[f]
        # 如果某一方原来是未关联状态，互换后需要更新状态
        # 只要有照片路径就设为 linked
        for m in (m1, m2):
            if m.get('panoramaPath') or m.get('originalPhotoPath'):
                m['status'] = 'linked'
            else:
                # 如果失去了照片，回退到 captured（如果原本有采集时间）否则 pending
                if m.get('captureTime') or m.get('timestamp'):
                    m['status'] = 'captured'
                else:
                    m['status'] = 'pending'
    ...
    def _on_swap(self):
        ...
        if len(selected) != 2:
            QMessageBox.warning(self, "提示", "请恰好选中 2 个采集点进行互换")
            return

        row1, row2 = selected
        marker1 = self._all_linked_markers[row1]['marker']
        marker2 = self._all_linked_markers[row2]['marker']

        self._swap_photo_data(marker1, marker2)
        self._refresh_table()

        name1 = marker1.get('customName', '') or marker1.get('id', '')
        name2 = marker2.get('customName', '') or marker2.get('id', '')
        QMessageBox.information(self, "互换成功", f"已互换:\n• {name1}\n• {name2}")

    def _on_cycle_shift(self, direction: int):
        """循环移位照片：direction=1 右移, -1 左移"""
        selected = self._get_selected_rows()
        if len(selected) < 2:
            QMessageBox.warning(self, "提示", "请至少选中 2 个采集点")
            return

        # 获取选中的 marker 数据
        markers = [self._all_linked_markers[r]['marker'] for r in selected]

        # 提取照片信息
        photo_infos = []
        for m in markers:
            photo_infos.append({
                'panoramaPath': m.get('panoramaPath', ''),
                'originalPhotoPath': m.get('originalPhotoPath', ''),
                'photos': m.get('photos', []),
        ...
        # 循环移位
        n = len(photo_infos)
        if direction == 1:  # 右移：最后一个移到第一个
            shifted = [photo_infos[-1]] + photo_infos[:-1]
        else:  # 左移：第一个移到最后一个
            shifted = photo_infos[1:] + [photo_infos[0]]

        # 写回
        for m, info in zip(markers, shifted):
            m['panoramaPath'] = info['panoramaPath']
            m['originalPhotoPath'] = info['originalPhotoPath']
            m['photos'] = info['photos']
            m['cameraFileName'] = info['cameraFileName']
            # 更新状态
            if m['panoramaPath'] or m['originalPhotoPath']:
                m['status'] = 'linked'
            else:
                m['status'] = 'captured' if (m.get('captureTime') or m.get('timestamp')) else 'pending'

        self._refresh_table()
        shift_name = "右移" if direction == 1 else "左移"
        QMessageBox.information(self, "完成", f"已循环{shift_name} {n} 个采集点的照片")

    def _on_custom_reorder(self):
        """打开自定义重排对话框"""
        selected = self._get_selected_rows()
        if len(selected) < 2:
            QMessageBox.warning(self, "提示", "请至少选中 2 个采集点进行自定义重排")
            return

        # 准备数据
        markers_data = []
        for r in selected:
            item = self._all_linked_markers[r]
            marker = item['marker']
            markers_data.append({
                'marker_id': marker.get('id', ''),
                'marker_name': marker.get('customName', '') or marker.get('id', ''),
                'floor_name': item['floor_name'],
                'photo_filename': marker.get('cameraFileName', '') or os.path.basename(marker.get('panoramaPath', '') or '无'),
                'panoramaPath': marker.get('panoramaPath', ''),
                'originalPhotoPath': marker.get('originalPhotoPath', ''),
                'photos': marker.get('photos', []),
                'cameraFileName': marker.get('cameraFileName', ''),
            })

        dialog = PhotoReorderDialog(markers_data, self)
        if dialog.exec() != QDialog.DialogCode.Accepted:
            return
        ...
        # 应用新顺序
        for m, info in zip(markers, new_order):
            m['panoramaPath'] = info['panoramaPath']
            m['originalPhotoPath'] = info['originalPhotoPath']
            m['photos'] = info['photos']
            m['cameraFileName'] = info['cameraFileName']
        ...
        self._refresh_table()
        QMessageBox.information(self, "完成", f"已按自定义顺序重新分配 {len(selected)} 个采集点的照片")

    def _preview_changes(self):
        """预览当前表格中的关联状态"""
        lines = []
        for item in self._all_linked_markers:
            marker = item['marker']
            name = marker.get('customName', '') or marker.get('id', '')
            photo = marker.get('cameraFileName', '') or os.path.basename(marker.get('panoramaPath', '') or '无')
            lines.append(f"[{item['floor_name']}] {name} → {photo}")
        QMessageBox.information(self, "当前关联预览", "\n".join(lines[:50]) + ("\n..." if len(lines) > 50 else ""))

    def _save_changes(self):
        """保存修改到项目"""
        if self.parent_manager:
            self.parent_manager._push_history()
            self.parent_manager._save_project()
            self.parent_manager._refresh_markers()
            self.parent_manager.import_status_label.setText("✅ 照片关联关系已调整并保存")
        self.accept()

class PanoramaManager(QMainWindow):
    # 方向更新信号（从HTTP服务器线程传递到主线程）
    direction_update = pyqtSignal(str, float)
    # 方向持久化保存信号（仅查看器对齐确认时触发，用于同步HTML）
    direction_saved = pyqtSignal(str, float)
    ...
    def _is_same_path(self, path1: str, path2: str) -> bool:
        """判断两个路径是否指向同一个物理位置（处理 junction、symlink、大小写）"""
    ...
    def _is_junction(self, path: str) -> bool:
        """检测 Windows junction / reparse point（避免 os.walk 递归进入）"""
    ...
    def _init_ui(self):
        ...
        self.floating_toolbar.photo_relation_clicked.connect(self._open_photo_relation_manager)
        # 标记点尺寸变化
        ...
        # 连接方向持久化保存信号（仅对齐确认时触发，用于同步查看器HTML）
        self.direction_saved.connect(self._on_direction_saved_from_viewer)
    ...
    def _update_floating_toolbar(self):
        ...
        # photo_relation 不需要选中点位，只要有项目数据即可启用
        if 'photo_relation' in self.floating_toolbar.buttons:
            has_project = self.project_data is not None
            self.floating_toolbar.buttons['photo_relation'].setEnabled(has_project)
    ...
    def _open_photo_relation_manager(self):
        """打开照片关联关系管理器"""
        if not self.project_data or not self.project_data.floors:
            QMessageBox.warning(self, "提示", "请先加载项目")
            return
        self._push_history()
        dialog = PhotoRelationManagerDialog(self.project_data, self.current_floor_id, self)
        dialog.exec()
    ...
    def _init_menu(self):
        ...
        # 按楼层拆分数据包
        split_by_floor_action = QAction("按楼层拆分数据包(&S)...", self)
        split_by_floor_action.triggered.connect(self.split_package_by_floor)
        file_menu.addAction(split_by_floor_action)
        ...
        tools_menu.addAction(photo_relation_action)
    ...
    def import_from_folder(self):
        ...
        支持两种模式：
        1. 传统模式：根目录下有 ZIP 数据包 + 照片文件夹（所有楼层照片混在一起）
        2. 按楼层拆分模式：根目录下有多个子文件夹，每个子文件夹包含该楼层的 ZIP + 照片
           结构示例：
           root_dir/
               ├── 1F/
               │   ├── 1F.zip
        ...
        try:
            ...
            zip_files = []
            photo_dirs = []
            floor_subdirs = {}  # {子文件夹名: {'zip': path, 'photo_dir': path}}
            ...
            for item in os.listdir(dir_path):
                ...
                if os.path.isfile(item_path) and item.lower().endswith('.zip'):
                    ...
                    # === 检测是否为"按楼层拆分"的子文件夹（内含 ZIP + 照片文件夹）===
                    sub_zips = []
                    sub_photos = []
                    try:
                        for sub_item in os.listdir(item_path):
                            sub_path = os.path.join(item_path, sub_item)
                            if os.path.isfile(sub_path) and sub_item.lower().endswith('.zip'):
                                sub_zips.append(sub_path)
                            elif os.path.isdir(sub_path):
                                sub_jpg_count = 0
                                for sr, sd, sf in os.walk(sub_path):
                                    for f in sf:
                                        if f.lower().endswith(('.jpg', '.jpeg')):
                                            sub_jpg_count += 1
                                    if sub_jpg_count > 0:
                                        break
                                if sub_jpg_count > 0:
                                    sub_photos.append((sub_path, sub_jpg_count))
                    except PermissionError:
                        pass  # 跳过无权限访问的文件夹

                    # 如果该子文件夹内同时有 ZIP 和照片，视为楼层数据包
                    if sub_zips and sub_photos:
                        floor_subdirs[item] = {
                            'zip': sub_zips[0],  # 取第一个 ZIP
                            'photo_dir': max(sub_photos, key=lambda x: x[1])[0]  # 取照片最多的
                        }
            ...
            # === 判断是否使用"按楼层拆分"导入模式 ===
            use_floor_mode = False
            if floor_subdirs:
                floor_names = list(floor_subdirs.keys())
                reply = QMessageBox.question(
                    self, "检测到按楼层拆分结构",
                    f"检测到以下楼层子文件夹（每个内含数据包+照片）：\n\n" +
                    "\n".join([f"• {name}" for name in floor_names]) +
                    f"\n\n共 {len(floor_names)} 个楼层，是否按楼层分别导入？\n\n"
                    "选择「是」：按楼层分别关联照片（避免楼层错位）\n"
                    "选择「否」：按传统模式导入（所有照片混在一起关联）",
                    QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No
                )
                if reply == QMessageBox.StandardButton.Yes:
                    use_floor_mode = True
            ...
            if use_floor_mode:
                self._import_from_folder_by_floor(dir_path, floor_subdirs)
                return
    ...
    def _import_from_folder_by_floor(self, root_dir: str, floor_subdirs: dict):
        """按楼层拆分模式导入项目
        ...
        导入流程：
        1. 解压每个楼层的 ZIP，提取楼层数据
        2. 合并所有楼层为一个项目
        3. 按楼层分别扫描对应的照片文件夹，避免楼层间照片错位关联
        """
        # 创建合并项目目录
        merge_dir = os.path.join(root_dir, "合并导入项目")
        counter = 1
        original_merge_dir = merge_dir
        while os.path.exists(merge_dir):
            merge_dir = f"{original_merge_dir}_{counter}"
            counter += 1
        ...
        try:
            # ========== 阶段1：合并所有楼层的项目数据 ==========
            for floor_name, floor_info in sorted(floor_subdirs.items()):
                zip_path = floor_info['zip']
                photo_dir = floor_info['photo_dir']

                # 解压 ZIP
                extract_dir = os.path.join(merge_dir, f'_extract_{floor_name}')
                os.makedirs(extract_dir, exist_ok=True)
                ...
                if not project_json_path:
                    print(f"[按楼层导入] 警告: {floor_name} 中未找到 project.json")
                    continue

                with open(project_json_path, 'r', encoding='utf-8') as f:
                    floor_project = json.load(f)

                # 获取楼层数据（split_package_by_floor 生成的 ZIP 通常只有一层）
                floors = floor_project.get('floors', [])
                if not floors and floor_project.get('markers'):
                    floors = [{
                        'id': f'legacy_{floor_name}',
                        'name': floor_name,
                        'order': 0,
                        'hasPlan': bool(floor_project.get('floorplan')),
                        'markers': floor_project['markers']
                    }]

                for floor_data in floors:
                    # 使用子文件夹名作为楼层名（保持用户命名习惯）
                    floor_data['name'] = floor_name
                    # 确保ID唯一，避免合并时冲突
                    floor_data['id'] = f"floor_{floor_name}_{int(datetime.now().timestamp()*1000)}"

                    # 复制平面图到合并目录
                    if floor_data.get('hasPlan', False):
                        src_dir = os.path.dirname(project_json_path)
                        plan_copied = False
                        for ext in ['.jpg', '.jpeg', '.png']:
                            # 尝试多种可能的平面图命名
                            for plan_name in [
                                f"floorplan_{floor_data.get('id', '')}{ext}",
                                f"floorplan_{floor_project.get('floors', [{}])[0].get('id', '')}{ext}",
                                f"floorplan{ext}"
                            ]:
                                src_plan = os.path.join(src_dir, plan_name)
                                if os.path.exists(src_plan):
                                    dst_plan = os.path.join(merge_dir, f"floorplan_{floor_data['id']}{ext}")
                        ...
                        if not plan_copied:
                            # 模糊匹配：任何包含 floorplan 的图片文件
                            for f in os.listdir(src_dir):
                                if 'floorplan' in f.lower() and f.lower().endswith(('.jpg', '.jpeg', '.png')):
                                    src_plan = os.path.join(src_dir, f)
                                    dst_plan = os.path.join(merge_dir, f"floorplan_{floor_data['id']}{os.path.splitext(f)[1]}")
                                    shutil.copy2(src_plan, dst_plan)
                                    break
            ...
            if not all_floors:
                QMessageBox.warning(self, "提示", "未能从任何楼层子文件夹中加载到有效数据")
                return

            # 创建合并后的项目
            merged_project = {
                'schemaVersion': '4.0',
                'projectName': f"合并导入_{os.path.basename(root_dir)}",
                'createdAt': datetime.now().isoformat(),
                'updatedAt': datetime.now().isoformat(),
                'timeOffset': 0,
                'calibrated': False,
                'photoBaseDir': root_dir,  # 照片基目录设为根目录（包含各楼层子文件夹）
                'floors': all_floors
            }

            project_json_path = os.path.join(merge_dir, 'project.json')
            with open(project_json_path, 'w', encoding='utf-8') as f:
                json.dump(merged_project, f, ensure_ascii=False, indent=2)

            # 加载合并项目
            self.project_dir = merge_dir
            self._load_project(project_json_path)

            # ========== 阶段2：按楼层分别导入照片（关键：避免楼层错位） ==========
            total_results = {'local_matched': 0, 'external_matched': 0, 'missing': 0}

            for floor_data in self.project_data.floors:
                floor_name = floor_data.get('name', '未知')
                floor_info = floor_subdirs.get(floor_name, {})
                photo_dir = floor_info.get('photo_dir')

                if not photo_dir or not os.path.exists(photo_dir):
                    print(f"[按楼层导入] 跳过 {floor_name}: 未找到对应照片文件夹")
                    continue

                # 更新状态提示
                self.import_status_label.setText(f"正在导入 {floor_name} 的照片...")
                QApplication.processEvents()

                # 同步导入该楼层的照片（只扫描该楼层对应的照片文件夹）
                results = self._import_photos_for_floor_sync(floor_data, photo_dir)

                total_results['local_matched'] += results.get('local_matched', 0)
                total_results['external_matched'] += results.get('external_matched', 0)
                total_results['missing'] += results.get('missing', 0)

                print(f"[按楼层导入] {floor_name} 完成: 关联{results.get('local_matched',0)+results.get('external_matched',0)}个, 缺失{results.get('missing',0)}个")
            ...
            total_linked = total_results['local_matched'] + total_results['external_matched']
            QMessageBox.information(
                self, "按楼层导入完成",
                f"已成功导入 {len(all_floors)} 个楼层的项目数据\n\n"
                f"🏠 本机拍摄自动关联: {total_results['local_matched']} 个\n"
                f"📱 外设照片时间匹配: {total_results['external_matched']} 个\n"
                f"✅ 总关联: {total_linked} 个\n"
                f"❌ 未找到: {total_results['missing']} 个\n\n"
                f"项目目录: {merge_dir}\n"
                f"各楼层照片已按文件夹名称匹配，避免楼层错位关联。"
            )

        finally:
            # 清理临时解压目录
            for d in temp_extract_dirs:
                shutil.rmtree(d, ignore_errors=True)

    def _import_photos_for_floor_sync(self, floor_data: dict, photo_dir: str) -> dict:
        """同步导入单个楼层的照片，阻塞直到线程完成

        关键差异：只扫描传入的 photo_dir，不会扫描其他楼层的照片，
        因此从根本上避免了楼层错位关联的问题。
        """
        if not photo_dir or not os.path.exists(photo_dir):
            return {'local_matched': 0, 'external_matched': 0, 'missing': 0}
        ...
        # 创建临时 floors 列表（只包含当前楼层，隔离其他楼层数据）
        temp_floors = [dict(floor_data)]
        photo_base_dir = self._safe_photo_base_dir() or photo_dir

        # 创建导入线程
        import_thread = PhotoImportThread(
            self.project_dir, photo_dir, temp_floors,
            self.project_data.timeOffset if self.project_data else 0,
            use_exif=True, threshold=300,
            photo_base_dir=photo_base_dir
        )

        results = {}
        import_thread.import_complete.connect(lambda r: results.update(r))
        import_thread.start()

        # 阻塞等待线程完成（同时保持 GUI 响应）
        while import_thread.isRunning():
            QApplication.processEvents()
            import_thread.wait(50)

        # 将导入结果合并回原始 floor_data
        if temp_floors and temp_floors[0].get('markers'):
            orig_markers = floor_data.get('markers', [])
            temp_markers = temp_floors[0]['markers']
            for i, marker in enumerate(orig_markers):
                if i < len(temp_markers):
                    temp_marker = temp_markers[i]
                    if temp_marker.get('status') == 'linked':
                        marker['status'] = 'linked'
                        marker['panoramaPath'] = temp_marker.get('panoramaPath', '')
                        marker['originalPhotoPath'] = temp_marker.get('originalPhotoPath', '')
                        marker['photos'] = temp_marker.get('photos', [])
                        marker['cameraFileName'] = temp_marker.get('cameraFileName', '')

        return results if results else {'local_matched': 0, 'external_matched': 0, 'missing': 0}
...
<body>
    ...
    <script>
        ...
        function enterAlignMode(marker, compass, label, hintBar) {
            ...
            const lockedHeading = normalizeAngle(lockedDir);
            ...
            console.log('[对齐模式] 进入，锁定 direction:', lockedDir, 'heading:', lockedHeading);
        }
        ...
        function confirmAlign(marker, compass, label, hintBar) {
            ...
            if (!window._syncInterval && viewer) {
                window._syncInterval = setInterval(() => {
                    if (!viewer) { clearInterval(window._syncInterval); window._syncInterval = null; return; }
                    try { sendDirectionUpdate(viewer.getYaw()); } catch(e) {}
                }, 100);
            }
        ...
        function loadFloor(index, targetMarkerIndex) {
            ...
            img.onload = () => {
                ...
                renderSectors(overlay, floor.markers, displayedWidth, displayedHeight);
                if (typeof targetMarkerIndex !== 'undefined') {
            loadPanorama(targetMarkerIndex);
        }
            };
...
function zoomIn() {
        ...
        function showPanoramaViewer(marker, photos) {
            ...
            // 不设置 config.yaw，全景始终从 yaw=0（照片正前方）开始
            // marker.direction 仅控制扇形在平面图上的指向，与全景视角无关
            ...
            if (indicator) {
                indicator.style.display = 'none';  // 强制隐藏时空转换提示
            }
            
            // 扇形方向 = _sectorBase（基准方向）+ 全景偏移（跟随旋转）
            // 基准方向 = marker.direction，可通过 align 模式更新
            window._sectorBase = (marker.direction !== null && marker.direction !== undefined && marker.direction !== '') 
                ? parseFloat(marker.direction) : 0;
            currentSectorDirection = window._sectorBase;
            window._lastYaw = 0;
            ...
            // 跟随全景旋转：扇形 = _sectorBase + 当前 yaw（全景从 yaw=0 开始）
            const sendDirectionUpdate = (yaw) => {
                if (compassAlignState.isAligning) return;
                let delta = normalizeAngle(yaw - (window._lastYaw || 0));
                window._lastYaw = yaw;
                if (Math.abs(delta) < 1) return;
                currentSectorDirection = normalizeAngle(currentSectorDirection + delta);
                const wrapper = document.getElementById('floorplanWrapper');
                if (wrapper) {
                    const floor = floors[currentFloorIndex];
                    const overlay = document.getElementById('floorplanOverlay');
        ...
        function showPhotoViewer(marker, photos) {
            ...
            currentPhotoIndex = (window._pendingPhotoIndex !== undefined) ? window._pendingPhotoIndex : 0;
    window._pendingPhotoIndex = undefined;
            photoScale = 1;
            photoOffsetX = 0;
            photoOffsetY = 0;
            renderPhoto(photos);
        ...
        window.addEventListener('resize', function() {
            ...
            resizeTimer = setTimeout(function() {
                if (viewMode === 'photo') {
                    window._pendingPhotoIndex = currentPhotoIndex;
        }
                loadFloor(currentFloorIndex, currentMarkerIndex);
            }, 250);
        });
...
function jumpToMarker(targetId) {
    ...
    def _on_direction_saved_from_viewer(self, marker_id: str, direction: float):
        """查看器对齐确认后同步更新 viewer HTML，确保刷新后仍保持新方向"""
        if not self.project_dir:
            return
        viewer_dir = os.path.join(self.project_dir, 'viewer')
        if os.path.exists(viewer_dir):
            ...
            if self.server_thread and self.server_thread.is_running:
                print(f"[查看器同步] 方向已持久化，viewer HTML 已更新 (marker={marker_id}, dir={direction:.1f}°)")
    ...
    def _on_marker_context_menu(self, marker_id: str, global_pos):
        ...
        try:
            ...
            swap_photo_action = QAction("互换关联照片", self)
            swap_photo_action.setToolTip("打开照片关联关系管理器")
            swap_photo_action.triggered.connect(self._open_photo_relation_manager)
            menu.addAction(swap_photo_action)            
            menu.addAction(relink_action)
            menu.addAction(delete_action)
            menu.addSeparator()
            menu.addAction(sector_action)
    ...
    def _export_standalone_viewer(self, export_dir=None):
        ...
        try:
            ...
            # ========== 1. 统一复制所有照片到 viewer/external_photos/ ==========
            ...
            # ---- 收集所有源目录并去重（防止 junction 导致重复复制）----
            sources = []  # [(label, src_path), ...]
            
            # 1a. 项目目录 photos/
            src_photos_dir = os.path.join(self.project_dir, 'photos')
            if os.path.exists(src_photos_dir):
                sources.append(('项目photos', src_photos_dir))
            
            # 1b. photo_base_dir（如果与已有源不同）
            if photo_base_dir and os.path.exists(photo_base_dir):
                if not any(self._is_same_path(photo_base_dir, s[1]) for s in sources):
                    sources.append(('photoBaseDir', photo_base_dir))
            
            # 1c. 项目目录 external_photos/（通常是 junction，去重跳过）
            src_external_dir = os.path.join(self.project_dir, 'external_photos')
            if os.path.exists(src_external_dir):
                if self._is_junction(src_external_dir):
                    junction_target = os.path.realpath(src_external_dir)
                    if not any(self._is_same_path(junction_target, s[1]) for s in sources):
                        sources.append(('external_photos', src_external_dir))
                elif not any(self._is_same_path(src_external_dir, s[1]) for s in sources):
                    sources.append(('external_photos', src_external_dir))
            
            # ---- 预检查磁盘空间（提前报错，避免复制到一半失败）----
            if sources:
                total_needed = 0
                for _, src in sources:
                    for root, dirs, files in os.walk(src):
                        # 关键：跳过 junction / symlink 子目录，防止递归统计
                        dirs[:] = [
                            d for d in dirs 
                            if not os.path.islink(os.path.join(root, d))
                            and not self._is_junction(os.path.join(root, d))
                ...
                dst_drive = os.path.splitdrive(viewer_dir)[0] or os.path.splitdrive(self.project_dir)[0] or 'C:'
                try:
                    free = shutil.disk_usage(dst_drive).free
                    if total_needed + 100*1024*1024 > free:  # 预留 100MB
                        QMessageBox.critical(
                            self, "磁盘空间不足",
                            f"预计需要 {total_needed/1024/1024:.0f} MB，目标盘仅剩 {free/1024/1024:.0f} MB。\n"
                            f"请清理磁盘空间或选择其他输出目录。"
                        )
                        progress.close()
                        return
            ...
            # ---- 执行复制（跳过 junction / symlink，防止递归遍历）----
            for label, src in sources:
                for root, dirs, files in os.walk(src):
                    # 关键修复：跳过 junction 和 symlink 子目录
                    dirs[:] = [
                        d for d in dirs
                        if not os.path.islink(os.path.join(root, d))
                        and not self._is_junction(os.path.join(root, d))
                        # 额外保护：在 photoBaseDir 下遇到 external_photos 子目录也跳过
                        and not (label == 'photoBaseDir' and d.lower() == 'external_photos')
                    ]
                            ...
                            if not os.path.exists(dst_file):
                                try:
                                    ...
                                    if getattr(e, 'winerror', None) == 112:
                                        QMessageBox.critical(
                                            self, "磁盘空间不足",
                                            f"复制照片时磁盘空间不足（来源：{label}）。\n"
                                            f"文件：{os.path.basename(src_file)}\n"
                                            f"请清理磁盘后重试。"
                                        )
                                        progress.close()
                                        return
            ...
            for floor_data in self.project_data.floors:
                floor_id = floor_data['id']
                copied = False
                search_dirs = [self.project_dir, os.path.join(self.project_dir, 'viewer')]
            ...
            pannellum_files = {
                ...
                'spritesheet@2x.png': 'https://cdn.jsdelivr.net/npm/pannellum@2.5.6/build/spritesheet@2x.png',
            }
            offline_ok = True
            ...
            # ========== 4. 生成 HTML 和 project.json ==========
            self._generate_viewer_html(viewer_dir)
            
            # 写入 project.json，统一路径为 external_photos/ 格式
            ...
            for floor in project_dict.get('floors', []):
                for marker in floor.get('markers', []):
                    if marker.get('status') == 'linked':
                        resolved = self._resolve_marker_panorama_path(marker, photo_base_dir)
                        if resolved:
                            marker['panoramaPath'] = resolved
                        marker.pop('originalPhotoPath', None)
            ...
            # 仅在全部离线资源下载成功时，才替换 HTML 中的 CDN 链接
            index_html = os.path.join(viewer_dir, 'index.html')
            if os.path.exists(index_html) and offline_ok:
            ...
            if not offline_ok:
                msg += "\n\n⚠️ Pannellum 离线资源下载失败，查看包将使用 CDN 链接（需联网）。"
    ...
    def split_package_by_floor(self):
        """将采集端导出的数据压缩包按楼层拆分

        拆分后结构：
        {zip_name}_按楼层拆分/
            {楼层名}/
                {楼层名}.zip          ← 内含 project.json + floorplan（项目数据）
                照片_需放置N张照片/   ← 空文件夹，提示用户放入照片

        用户只需将照片拖入对应楼层的照片文件夹，
        然后使用「文件 → 从文件夹导入」选择该楼层文件夹即可自动关联。
        """
        file_path, _ = QFileDialog.getOpenFileName(
            self, "选择要拆分的采集端数据包", "", "ZIP 文件 (*.zip)"
        )
        if not file_path:
            return
        ...
        try:
            zip_dir = os.path.dirname(file_path)
            zip_name = os.path.splitext(os.path.basename(file_path))[0]
            output_base = os.path.join(zip_dir, f"{zip_name}_按楼层拆分")

            counter = 1
            original_output = output_base
            while os.path.exists(output_base):
                output_base = f"{original_output}_{counter}"
                counter += 1
            os.makedirs(output_base, exist_ok=True)

            # 解压 ZIP 到临时目录
            temp_dir = os.path.join(tempfile.gettempdir(), f"suixi_split_{int(datetime.now().timestamp()*1000)}")
            os.makedirs(temp_dir, exist_ok=True)
            ...
            if not project_json_path:
                QMessageBox.critical(self, "错误", "未在数据包中找到 project.json 文件")
            ...
            if not floors:
                QMessageBox.warning(self, "提示", "该项目没有楼层数据，无需拆分")
                shutil.rmtree(temp_dir, ignore_errors=True)
                return

            project_name = project_data.get('projectName', '未命名项目')
            ...
            for floor in floors:
                ...
                floor_name = floor.get('name', f'楼层{floor_id}')
                markers = floor.get('markers', [])

                # 计算需要放置照片的采集点数量
                need_photo_count = sum(
                    1 for m in markers
                    if m.get('status') in ('captured', 'pending', 'missing')
                )
                linked_count = sum(
                    1 for m in markers if m.get('status') == 'linked'
                )

                safe_floor_name = "".join(c for c in floor_name if c.isalnum() or c in (' ', '-', '_')).strip()
                if not safe_floor_name:
                    safe_floor_name = floor_id

                # ===== 创建楼层文件夹 =====
                floor_folder = os.path.join(output_base, safe_floor_name)
                os.makedirs(floor_folder, exist_ok=True)

                # 在楼层文件夹内创建照片文件夹（空文件夹，提示用户放照片）
                photo_folder_name = f"照片_需放置{need_photo_count}张照片"
                if linked_count > 0:
                    photo_folder_name += f"_已关联{linked_count}张"
                photo_dir = os.path.join(floor_folder, photo_folder_name)
                os.makedirs(photo_dir, exist_ok=True)

                # 在楼层文件夹内组装 ZIP（只包含项目数据，不包含照片文件夹）
                floor_temp = os.path.join(temp_dir, f"floor_{floor_id}")
                if os.path.exists(floor_temp):
                    shutil.rmtree(floor_temp, ignore_errors=True)
                os.makedirs(floor_temp, exist_ok=True)

                # 写入该楼层的独立 project.json
                floor_project = {
                    'schemaVersion': project_data.get('schemaVersion', '4.0'),
                    'projectName': f"{project_name}_{floor_name}",
                    'createdAt': project_data.get('createdAt', datetime.now().isoformat()),
                    'updatedAt': datetime.now().isoformat(),
                    'timeOffset': project_data.get('timeOffset', 0),
                    'calibrated': project_data.get('calibrated', False),
                    'photoBaseDir': '',
                    'floors': [floor]
                }
                with open(os.path.join(floor_temp, 'project.json'), 'w', encoding='utf-8') as f:
                    json.dump(floor_project, f, ensure_ascii=False, indent=2)

                # 复制平面图到临时目录（ZIP 内需要）
                plan_copied = False
                search_dirs = [src_project_dir, os.path.join(src_project_dir, 'viewer')]
                for search_dir in search_dirs:
                    if plan_copied:
                        break
                    for ext in ['.jpg', '.jpeg', '.png']:
                        src_plan = os.path.join(search_dir, f"floorplan_{floor_id}{ext}")
                        if os.path.exists(src_plan):
                            shutil.copy2(src_plan, os.path.join(floor_temp, f"floorplan_{floor_id}{ext}"))
                            plan_copied = True
                            break

                # 复制已关联照片到楼层文件夹的照片文件夹（作为示例/已有照片）
                copied_photos = 0
                for marker in markers:
                    if marker.get('status') == 'linked':
                        photo_path = marker.get('panoramaPath') or marker.get('originalPhotoPath', '')
                        if not photo_path:
                            continue
                        if photo_path.startswith('external_photos/'):
                            photo_path = photo_path[len('external_photos/'):]
                        possible_paths = [
                            os.path.join(src_project_dir, photo_path),
                            os.path.join(src_project_dir, 'external_photos', photo_path),
                            os.path.join(src_project_dir, 'photos', os.path.basename(photo_path)),
                            os.path.join(src_project_dir, 'viewer', 'external_photos', os.path.basename(photo_path)),
                        ]
                        for pp in possible_paths:
                            if os.path.exists(pp):
                                try:
                                    shutil.copy2(pp, os.path.join(photo_dir, os.path.basename(pp)))
                                    copied_photos += 1
                                except Exception:
                                    pass
                                break

                # 将该楼层临时文件夹打包为 ZIP，放到楼层文件夹内
                zip_filename = f"{safe_floor_name}.zip"
                zip_output_path = os.path.join(floor_folder, zip_filename)
                with zipfile.ZipFile(zip_output_path, 'w', zipfile.ZIP_DEFLATED) as zfw:
                    for root, dirs, files in os.walk(floor_temp):
                        for f_name in files:
                            abs_path = os.path.join(root, f_name)
                            arc_name = os.path.relpath(abs_path, floor_temp)
                            zfw.write(abs_path, arc_name)

                split_count += 1
                photo_hint = f"需照片 {need_photo_count} 个"
                if copied_photos > 0:
                    photo_hint += f"，已含 {copied_photos} 张示例照片"
                if linked_count > 0 and copied_photos == 0:
                    photo_hint += f"（原有关联 {linked_count} 张未找到）"
                report_lines.append(
                    f"• {safe_floor_name}/: {len(markers)} 个点位（{photo_hint}）"
                )
...
"""
            for line in report_lines:
                floor_name = line.split('/:')[0].replace('• ', '').strip()
                report += f"    {floor_name}/\n"
                report += f"        ├── {floor_name}.zip\n"
                report += f"        └── 照片_需放置.../\n"

            report += """
💡 使用说明：
1. 每个楼层已创建独立文件夹，内含 ZIP 项目包和照片文件夹
2. 将对应楼层的照片直接拖入「照片_需放置N张照片」文件夹
3. 使用管理器「文件 → 从文件夹导入」选择该楼层文件夹即可自动关联
4. ZIP 包内含 project.json 和平面图，可直接被识别
"""
            QMessageBox.information(self, "拆分成功", report)

        except Exception as e:
            import traceback
            tb = traceback.format_exc()
            print(tb)
            QMessageBox.critical(self, "拆分失败", f"错误: {e}\n\n{tb[:800]}")

    def _merge_projects(self):
        """合并多个项目数据包（支持文件夹/ZIP），导出为管理端项目或独立查看包"""
        ...
        if not sources or not output_dir or not project_name:
            return

        temp_dirs = []
        try:
            progress = QProgressDialog("正在合并项目...", "取消", 0, 100, self)
            progress.setWindowModality(Qt.WindowModality.ApplicationModal)
            progress.setWindowTitle("合并项目")
            progress.show()

            all_floors = []
            merged_photos_dir = os.path.join(tempfile.gettempdir(), f"suixi_merge_photos_{int(datetime.now().timestamp()*1000)}")
            os.makedirs(merged_photos_dir, exist_ok=True)
            photo_counter = 0
            ...
            for src_idx, (src_path, is_zip) in enumerate(sources):
                ...
                # 确定项目根目录
                if is_zip:
                    temp_dir = os.path.join(tempfile.gettempdir(), f"suixi_merge_src_{src_idx}_{int(datetime.now().timestamp()*1000)}")
                    os.makedirs(temp_dir, exist_ok=True)
                    temp_dirs.append(temp_dir)
                    with zipfile.ZipFile(src_path, 'r') as zf:
                        zf.extractall(temp_dir)
                    src_project_dir = None
                    for root, dirs, files in os.walk(temp_dir):
                        if 'project.json' in files:
                            src_project_dir = root
                            break
                else:
                    src_project_dir = None
                    for root, dirs, files in os.walk(src_path):
                        if 'project.json' in files:
                            try:
                                ...
                                if test_data.get('floors') or test_data.get('markers'):
                                    src_project_dir = root
                                    break
                            except Exception:
                                continue
                ...
                if not src_project_dir or not os.path.exists(os.path.join(src_project_dir, 'project.json')):
                    print(f"[合并] 警告: 在源 {src_path} 中未找到有效的 project.json")
                    continue

                with open(os.path.join(src_project_dir, 'project.json'), 'r', encoding='utf-8') as f:
                    src_data = json.load(f)

                src_floors = src_data.get('floors', [])
                if not src_floors and src_data.get('markers'):
                    src_floors = [{
                        'id': 'legacy_floor',
                        'name': '默认楼层',
                        'order': 0,
                        'hasPlan': bool(src_data.get('floorplan')),
                        'markers': src_data['markers']
                    }]

                # 处理楼层ID冲突：如果ID已存在，添加后缀
                existing_floor_ids = {f['id'] for f in all_floors}
                for floor in src_floors:
                    orig_floor_id = floor['id']
                    if orig_floor_id in existing_floor_ids:
                        suffix = 1
                        new_id = f"{orig_floor_id}_m{suffix}"
                        while new_id in existing_floor_ids:
                            suffix += 1
                            new_id = f"{orig_floor_id}_m{suffix}"
                        floor['id'] = new_id
                    existing_floor_ids.add(floor['id'])

                    # 复制该楼层的平面图到合并照片目录（临时存放，后面统一处理）
                    if floor.get('hasPlan'):
                        for ext in ['.jpg', '.jpeg', '.png']:
                            plan_src = os.path.join(src_project_dir, f"floorplan_{orig_floor_id}{ext}")
                            if not os.path.exists(plan_src):
                                plan_src = os.path.join(src_project_dir, 'viewer', f"floorplan_{orig_floor_id}{ext}")
                            if os.path.exists(plan_src):
                                plan_dst = os.path.join(merged_photos_dir, f"floorplan_{floor['id']}{ext}")
                                if not os.path.exists(plan_dst):
                                    shutil.copy2(plan_src, plan_dst)
                                break

                    # 处理该楼层的照片：复制到合并目录，并重写路径
                    for marker in floor.get('markers', []):
                        if marker.get('status') != 'linked':
                            continue
                        ...
                        # 查找照片实际位置
                        photo_src = None
                        orig_path = marker.get('originalPhotoPath', '')
                        ...
                        if orig_path and os.path.exists(orig_path):
                            ...
                            candidates = [
                                os.path.join(src_project_dir, pano_path),
                                os.path.join(src_project_dir, 'viewer', pano_path),
                                os.path.join(src_project_dir, 'external_photos', os.path.basename(pano_path)),
                                os.path.join(src_project_dir, 'photos', os.path.basename(pano_path)),
                            ]
                            # 如果 panoramaPath 包含 external_photos/，尝试从上级目录找
                            if pano_path.startswith('external_photos/'):
                                sub = pano_path[len('external_photos/'):]
                                candidates.append(os.path.join(src_project_dir, pano_path))
                                candidates.append(os.path.join(os.path.dirname(src_project_dir), sub))
                                candidates.append(os.path.join(os.path.dirname(src_project_dir), 'external_photos', sub))
                            for c in candidates:
                        ...
                        if not photo_src:
                            print(f"[合并警告] 找不到点位 {marker.get('id')} 的照片")
                        ...
                        # 生成唯一目标文件名
                        basename = os.path.basename(photo_src)
                        name_root, name_ext = os.path.splitext(basename)
                        dst_name = basename
                        conflict_idx = 1
                        while dst_name in used_photo_names:
                            dst_name = f"{name_root}_{conflict_idx}{name_ext}"
                            conflict_idx += 1
                        used_photo_names.add(dst_name)

                        # 复制到合并照片目录
                        try:
                            shutil.copy2(photo_src, os.path.join(merged_photos_dir, dst_name))
                            photo_counter += 1
                        except Exception as e:
                            print(f"[合并错误] 复制照片失败: {e}")
                            marker['status'] = 'missing'
                            marker['panoramaPath'] = ''
                            marker['photos'] = []
                            continue
                        ...
                        # 重写 marker 路径为统一格式
                        marker['panoramaPath'] = 'external_photos/' + dst_name
                        marker['photos'] = ['external_photos/' + dst_name]
                        marker.pop('originalPhotoPath', None)

                    all_floors.append(floor)

                progress.setValue(5 + int((src_idx + 1) / len(sources) * 40))
                QApplication.processEvents()

            if not all_floors:
                QMessageBox.warning(self, "提示", "没有有效的楼层数据可以合并")
                return

            # ========== 第二步：组装合并后的项目 ==========
            progress.setLabelText("正在组装合并项目...")
            progress.setValue(50)
            ...
            # 按 order 排序
            merged_project['floors'].sort(key=lambda f: f.get('order', 0), reverse=True)
            ...
            if output_type == 'project':
                ...
                counter = 1
                orig = output_project_dir
                while os.path.exists(output_project_dir):
                    output_project_dir = f"{orig}_{counter}"
                    counter += 1
                os.makedirs(output_project_dir, exist_ok=True)

                # 写入 project.json
                with open(os.path.join(output_project_dir, 'project.json'), 'w', encoding='utf-8') as f:
                    json.dump(merged_project, f, ensure_ascii=False, indent=2)
                ...
                for floor in all_floors:
                    fid = floor['id']
                    for ext in ['.jpg', '.jpeg', '.png']:
                        src = os.path.join(merged_photos_dir, f"floorplan_{fid}{ext}")
                        if os.path.exists(src):
                            shutil.copy2(src, os.path.join(output_project_dir, f"floorplan_{fid}{ext}"))
                            break

                # 复制照片到 external_photos/
                dst_photos = os.path.join(output_project_dir, 'external_photos')
                os.makedirs(dst_photos, exist_ok=True)
                for f in os.listdir(merged_photos_dir):
                    if f.startswith('floorplan_'):
                        continue
                    shutil.copy2(os.path.join(merged_photos_dir, f), os.path.join(dst_photos, f))

                # 更新 photoBaseDir
                merged_project['photoBaseDir'] = dst_photos
                with open(os.path.join(output_project_dir, 'project.json'), 'w', encoding='utf-8') as f:
                    json.dump(merged_project, f, ensure_ascii=False, indent=2)

                # 可选：自动生成查看包
                if auto_export:
                    self.project_dir = output_project_dir
                    self.project_data = Project.from_dict(merged_project)
                    self._export_standalone_viewer(output_dir)

                progress.setValue(100)
                QMessageBox.information(self, "合并成功",
                    f"管理端项目已导出到:\n{output_project_dir}\n\n"
                    f"合并楼层: {len(all_floors)}\n"
                    f"合并照片: {photo_counter} 张")

            else:
                # 独立查看包：直接生成完整 viewer 目录
                package_dir = os.path.join(output_dir, f"{project_name}_查看包")
                counter = 1
                orig = package_dir
                while os.path.exists(package_dir):
                    package_dir = f"{orig}_{counter}"
                    counter += 1
                os.makedirs(package_dir, exist_ok=True)
                ...
                # 复制照片到 viewer/external_photos/
                dst_photos = os.path.join(viewer_dir, 'external_photos')
                os.makedirs(dst_photos, exist_ok=True)
                for f in os.listdir(merged_photos_dir):
                    if f.startswith('floorplan_'):
                        continue
                ...
                for floor in all_floors:
                    ...
                    for ext in ['.jpg', '.jpeg', '.png']:
                        src = os.path.join(merged_photos_dir, f"floorplan_{fid}{ext}")
                        if os.path.exists(src):
                            shutil.copy2(src, os.path.join(viewer_dir, f"floorplan_{fid}{ext}"))
                            break

                # 写入 project.json（路径已统一为 external_photos/）
                with open(os.path.join(viewer_dir, 'project.json'), 'w', encoding='utf-8') as f:
                    json.dump(merged_project, f, ensure_ascii=False, indent=2)

                # 生成 HTML
                self.project_data = Project.from_dict(merged_project)
                self.project_dir = package_dir  # 临时设置，供 _generate_viewer_html 使用
                self._generate_viewer_html(viewer_dir)
                ...
                QMessageBox.information(self, "合并成功", msg)
            ...
            if 'merged_photos_dir' in dir() and os.path.exists(merged_photos_dir):
                shutil.rmtree(merged_photos_dir, ignore_errors=True)
